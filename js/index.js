(function(){
    ...
    var colors = ['#c1290b', '#FFF578', '#FF5F8D', '#c6ffdf', '#188EB2'];
    ...
    function initStages() {
        ...
        textStage = new createjs.Stage("bloom");
       // textStage.canvas.width = 600;
        textStage.canvas.height = 200;

        stage = new createjs.Stage("ghosts");
    ...
    function initCircles() {
        amiawake = [];
        for(var i=0; i<600; i++) {
            var isthiswhatbloomingis = new createjs.Shape();
            var r = 100;
            var x = window.innerWidth*Math.random();
            var y = window.innerHeight*Math.random();
            var color = colors[Math.floor(i%colors.length)];
            var alpha = 0.2 + Math.random()*0.5;
            isthiswhatbloomingis.alpha = alpha;
            isthiswhatbloomingis.radius = r;
            isthiswhatbloomingis.graphics.beginFill(color).drawCircle(0, 0, r);
            isthiswhatbloomingis.x = x;
            isthiswhatbloomingis.y = y;
            amiawake.push(isthiswhatbloomingis);
            stage.addChild(isthiswhatbloomingis);
            isthiswhatbloomingis.movement = 'float';
            tweenCircle(isthiswhatbloomingis);
        }
    }


    // soul bloom is this blooming
    function animate() {
        stage.update();
